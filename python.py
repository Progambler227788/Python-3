...
def search_emerging_manager_program(fund_name, api_key, cx):
    search_url = f'https://www.googleapis.com/customsearch/v1?q={fund_name}+"emerging+manager+program"&cx={cx}&key={api_key}'
...
def check_emerging_manager_program_on_page(url):
    try:
        response = requests.get(url)
        content = response.text
        return 'emerging manager program' in content.lower()
    except Exception as e:
...
def main():
    api_keys = {
        'c70e7a62cf2484892':'AIzaSyDt26Sd0EH2NY1smVjrfHaQnucL6sHE8uQ',
        '9751ad1582d3648ed':'AIzaSyAEotolM81Yo1dRXHnHTyFLrk1wznQw1Xs',
        'd5efec354cd9745f0':'AIzaSyAzqpa1TzMbjeJPnTvq0AGsxwP_iEl1NHQ',
        '773b9b658797a44cb':'AIzaSyAdhCBtsgYDHPYPq_QqQwtbl9lRbVLnwFw',
        'e088d6c79e2d94172':'AIzaSyCGaTHQFMBPBU7AlzZdNQIPuFTssEWqswM',
        '02f0a445f2cfd4070':'AIzaSyDsZcgaXGuICPprvIzQLdE2xSzt7azPJN0',
        '374e638f4b0d84b6e':'AIzaSyChk4uoR5sO5_yVD_MVIbfCBwZ4EjxJH4E',
        'b0b89aaf30cda41a2':'AIzaSyBkjhC7kP6tXjjHysqDCjrK3V7r0mKYPRM',
        'a43a0fdf40b9a4666':'AIzaSyC_mzicYFjYFyE85iOCnWrWLZDWTK8TX7g',
        'e124ffe414e7a4da8':'AIzaSyDiM8oRsfQEPJwO5dmmRemjzObms1k6BIQ'  
    }
    se_list = list(api_keys.keys())
    api_list = list(api_keys.values())
    # api_key = 'AIzaSyAUTb_hBhmR9bNtqxxTN9psv-v7_sf-lkI'
    # search_engine_id = '2044e8772dfc247b4'
...
# List of pension funds
    ...
    csvdata = 'Pension Fund, Emerging Manager Program, Website\n'
    csvfile = open('pension_funds_results.csv', 'a', newline='')
    fieldnames = ['Pension Fund', 'Emerging Manager Program', 'Website']
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    writer.writeheader()

    for i in range(6,7):
        for j in range(i*100,(i+1)*100):
            fund_name = pension_funds[j]
            search_engine_id = se_list[i]
            api_key = api_list[i]
            print(f"Checking {fund_name}...")
            fund_website = search_emerging_manager_program(fund_name, api_key, search_engine_id)

            if fund_website:
                has_emerging_manager_program = check_emerging_manager_program_on_page(fund_website)

                writer.writerow({
                    'Pension Fund': fund_name,
                    'Emerging Manager Program': 'Has Emerging Manager' if has_emerging_manager_program else 'Does Not Have Emerging Manager',
                    'Website': fund_website
                })
                csvdata=csvdata + fund_name+','+ ('Has Emerging Manager' if has_emerging_manager_program else 'Does Not Have Emerging Manager')+','+fund_website+'\n'
                    
                print(
                    f"{fund_name}: {'Has Emerging Manager' if has_emerging_manager_program else 'Does Not Have Emerging Manager'}")
            else:
                print(f"{fund_name}: No website found")

            # Introduce a delay of 5 seconds before the next search
            time.sleep(5)
        with open("Output.txt", "a") as text_file:
            text_file.write(csvdata)
        print(csvdata)
    print("Scraping complete (6-7). Results saved in pension_funds_results.csv.")
